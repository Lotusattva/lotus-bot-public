...
int main() {
    ...
    bot.on_message_create([](const message_create_t& event)  {
        ...
        string lowercase_no_space{string_view{event.msg.content} 
                    | views::filter([](const char& c){return c != ' ';}) 
                    | views::transform([](const char& c){return std::tolower(c);}) 
                    | ranges::to<string>()};
        ...
        while ((pos = lowercase_no_space.find("feet", pos)) != string::npos) {
            ...
            if (pos < lowercase_no_space.size() && std::isdigit(lowercase_no_space[pos])) {
                ...
                feet_count += max(std::stoi(lowercase_no_space.substr(pos, end_pos - pos)), 0); // Use max() to prevent integer overflow
