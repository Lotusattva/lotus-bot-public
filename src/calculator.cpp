...
command_option calculator_commands() {
    return command_option{ co_sub_command_group, "calc", "cultivation calculator" }
        .add_option(command_option{ co_sub_command, "interactive",
            "interactive step-by-step calculator" }
        )
        .add_option(command_option{ co_sub_command, "percent",
            "report percent progress during an interactive calc session" }
            .add_option(command_option{ co_number, "percentage",
                "percentage displayed on top left corner of the screen", true }
                .set_min_value(0.0)
                .set_max_value(1000.0))
        )
        .add_option(command_option{ co_sub_command, "cosmosapsis",
            "report cosmosapsis during an interactive calc session" }
            .add_option(command_option{ co_number, "cosmosapsis_val",
                    "cosmosapsis displayed in your abode", true }
                .set_min_value(0.0))
        )
        .add_option(command_option{ co_sub_command, "respira",
            "report respira during an interactive calc session" }
            .add_option(command_option{ co_integer, "respira_exp",
                    "respira exp displayed in your abode", true }
                .set_min_value(0))
            .add_option(command_option{ co_integer, "daily_attempts",
                    "number of daily respira attempts", true }
                .set_min_value(0))
        );
}
...
task<void> calc_ask_percent_progress(const button_click_t& event) {
    ...
    calculator_client_t& client{ calc_sessions[event.command.usr.id].second };
    string client_info{ "You have entered:\n"
...
task<void> calc_ask_aura_gem(const button_click_t& event) {
    ...
    string client_info{ "You have entered:\n"
        "- Major Stage: " + string{MAJOR_STAGE_STR[calc_sessions[event.command.usr.id].second.major_stage]} + "\n"
        "- Minor Stage: " + string{MINOR_STAGE_STR[calc_sessions[event.command.usr.id].second.minor_stage]} + "\n"
        "- Percent Progress: " + to_string(calc_sessions[event.command.usr.id].second.percent_progress) + "%\n"
        "- Cosmosapsis: " + to_string(calc_sessions[event.command.usr.id].second.cosmosapsis) + "\n\n" };

    static component aura_gem_quality_selectmenu{ component()
        .set_type(cot_action_row)
...
task<void> calc_ask_respira(const button_click_t& event) {
    ...
    string client_info{ "You have entered:\n"
        ...
        "- Minor Stage: " + string{MINOR_STAGE_STR[client.minor_stage]} + "\n"
        "- Percent Progress: " + to_string(client.percent_progress) + "%\n"
        "- Cosmosapsis: " + to_string(client.cosmosapsis) + "\n"
        "- Aura Gem Quality: " + string{QUALITY_STR[client.aura_gem_quality]} + "\n\n" };

    component text_display{ component()
        .set_type(cot_text_display)
        .set_content(client_info +
            "Please input the amount of exp per respira attempt and "
            "the number of daily respira attempts via `/calc respira` command.\n\n"
            "We will continue once your input is received."
        )
    };
...
task<void> process_respira(const slashcommand_t& event) {
    ...
    calculator_client_t& client{ it->second.second };
    client.respira_exp = static_cast<exp_t>(get<int64_t>(event.get_parameter("respira_exp")));
    client.daily_respira_attempts = static_cast<exp_t>(get<int64_t>(event.get_parameter("daily_attempts")));

    component text_display{ component()
        .set_type(cot_text_display)
        .set_content("Your respira exp is set to " + to_string(client.respira_exp) + ", "
            "and daily attempts are set to " + to_string(client.daily_respira_attempts) + ".\n\n"
            "Please click **NEXT** to continue, or use `/calc respira` to update your progress.\n\n")
    };
