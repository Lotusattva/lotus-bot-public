...
constexpr uint64_t hash_string(string_view str) {
    uint64_t hash = 5381;
    for (char c : str) { hash = ((hash << 5) + hash) + static_cast<unsigned char>(c); }
    return hash;
}
...
constexpr inline const exp_t* const GATE_EXP_REQ[NUM_MAJOR_STAGES][NUM_MINOR_STAGES]{
    ...
    {                // INCARNATION
     ...
     (const exp_t[]){// MIDDLE
                     1412392, 1804723, 2170898, 2537074, 2903249, 3269425, 3635601, 4027931,
                     4394107},
     (const exp_t[]){// LATE
                     1483013, 1853766, 2286312, 2595272, 3027818, 3460363, 3769324, 4201870,
                     4572623, 4943376, 5252337, 5623090, 5932051, 6241012, 6549973}},
    {                // VOIDBREAK
     (const exp_t[]){// EARLY
...
constexpr inline const int RESPIRA_MULT[]{1, 2, 5, 10};

constexpr inline const double RESPIRA_MULT_CHANCE[]{0.55, 0.3, 0.1475, 0.0025};
