...
constexpr inline chrono::hours DAILY_RESET_TIME{8 + 5};  // 8 am + 5 hours offset
...
enum class calculator_error_t {
    ...
    MYRIMON_NOW,     // The player can breakthrough to the next major stage now if they consume the
                     // amount of myrimon fruits they have set in the calculator client.
 ...
 * - The player has used all their daily pill attempts this server day (pill attempts for this
 * server day are not included in the calculation).
 * - The player has used all their daily respira attempts this server day (respira attempts for this
 * server day are not included in the calculation).
 * - The player has used their artifact daily recharges this server day (artifact recharges for this
 * server day are not included in the calculation).
 ...
 * - The player will consume their myrimon fruits by the number they have set in the calculator
 * client in the near future.
 *
 * @param client The calculator client.
 * @return An expected value containing the estimated time to breakthrough in hours, or an error if
